...
export class DemSource {
  ...
  analysisProtocolUrlBase: string;
  manager: DemManager;
  ...
  setupMaplibre = (maplibre: {
    ...
    maplibre.addProtocol(this.analysisProtocolId, this.analysisProtocol);
  };
  ...
  /**
   * Callback to be used with maplibre addProtocol to generate slope/aspect raster tiles according
   * to options encoded in the tile URL pattern generated by `analysisProtocolUrl`.
   */
  analysisProtocolV4: AddProtocolAction = async (
    request: RequestParameters,
    abortController: AbortController,
  ) => {
    const timer = new Timer("main");
    ...
    try {
      const [z, x, y] = this.parseUrl(request.url);
      const options = decodeAnalysisOptions(request.url);
      const data = await this.manager.fetchAnalysisTile(
        z,
        x,
        y,
        options,
  ...
  analysisProtocol: V3OrV4Protocol = v3compat(this.analysisProtocolV4);
  ...
  /**
   * Returns a URL with the analysis maplibre protocol prefix and encoded analysis options.
   */
  analysisProtocolUrl = (options: GlobalAnalysisTileOptions) =>
    `${this.analysisProtocolUrlBase}?${encodeAnalysisOptions(options)}`;
}
