 ...
 */
function smoothChaikin(points: number[], iterations: number = 3): number[] {
  if (points.length <= 4) return points;
...
function applySmoothing(
  ...
  switch (method) {
    ...
    case "catmull-rom":
      return smoothCatmullRom(points, 0.5, segments);
    case "bezier":
      return smoothBezier(points, segments);
    default:
      return points;
  }
}
 ...
 * @param smooth Apply smoothing to contour lines: 'none' = no smoothing, 'linear' = weighted average, 'chaikin' = corner cutting, 'catmull-rom' = spline interpolation, 'bezier' = high-res linear interpolation (default none)
 * @param smoothIterations Number of times to apply smoothing (default 3 for iterative methods)
 * @param smoothSegments Number of interpolated points per segment (default 8 for subdivision methods)
...
export default function generateIsolines(
  ...
  // REMOVED round parameter from signature
  if (!interval) {
    return {};
  }
  // The final output MUST be rounded to integer coordinates for vector tile encoding
  // This is a necessary step and the cause of the original gap issue was rounding
  // during the connection process in the Fragment class.
  const ROUND_FINAL_OUTPUT = true;
            ...
            if ((g = fragmentByStart.get(endIndex))) {
              ...
              if (f === g) {
                ...
                if (!f.isEmpty()) {
                  ...
                  // Apply smoothing if enabled, then round
  ...
  // Process fragments that didn't close (open lines)
  for (const [level, fragmentByStart] of fragmentByStartByLevel.entries()) {
    ...
    for (const value of fragmentByStart.values()) {
      if (!value.isEmpty()) {
        ...
        if (smooth !== "none") {
          line = applySmoothing(line, smooth, smoothIterations, smoothSegments);
        }
        if (ROUND_FINAL_OUTPUT) {
          line = line.map((coord) => Math.round(coord));
        }
