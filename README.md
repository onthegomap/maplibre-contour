...
- **Node.js 20** is required (the `canvas` native dependency does not compile on Node 22+).
  A `.tool-versions` file is provided for [mise](https://mise.jdx.dev/) / asdf users.
...
You can also generate analysis raster overlays (for example slope/aspect) from the same DEM source:
...
map.addSource("analysis-source", {
  ...
  tiles: [
    demSource.analysisProtocolUrl({
      mode: "slope", // "slope" | "aspect"
      units: "degrees", // "degrees" | "percent" (for slope)
      ramp: "default", // named ramp or JSON-encoded expression subset
      alpha: 0.55, // per-pixel output alpha multiplier [0..1]
      smooth: false,
      exaggeration: 1,
    }),
  ],
...
- [`analysis/analysis-kernel.ts`](./src/analysis/analysis-kernel.ts) computes seam-safe per-pixel slope/aspect scalar tiles from the same DEM-neighbor pipeline.
- [`analysis/analysis-color.ts`](./src/analysis/analysis-color.ts) maps scalar values to RGBA using `step`/`interpolate` expression subsets.
- [`analysis/analysis-encode.ts`](./src/analysis/analysis-encode.ts) encodes analysis RGBA tiles into PNG bytes returned through the analysis protocol.
